...
# 专属输出目录 (路线 A: 实际 ET + 废除代偿)
output_dir     = raw"C:\Users\26332\OneDrive\Desktop\kong mission\mechin learning\code\infiltration\Physics_Forward_反演ET-改2/"
...
function compute_exponential_sink!(sink, θ, param, exp_weights, ET_actual_cm_h, dz_layers, N_layers)
    for i in 1:N_layers
        # 🌟 物理防爆底线：如果该层接近残余含水率，直接断水
        # 并且配额直接“作废”，绝不转嫁给深层！
        if θ[i] < (param.θ_sat[i] * 0.08)
            sink[i] = 0.0
        else
            # 🌟 废除 w_norm 归一化！
            # 仅仅按照 Jackson 根系比例扣除该层本该承担的真实份额
...
et_val_col  = :ET_mm  
println("🔍 已成功锁定时间列 [$et_time_col] 和 实际蒸散发列 [$et_val_col]")
...
# ====================================================================
# 5. 核心循环：反演参数 + 实际ET + 无代偿 + 连续时间轴修复
# ====================================================================
println("🚀 [3/4] 启动 反演参数+实际ET(无代偿) 物理演化 (共 $total_sites 个站点) ...")
    ...
    lock(io_lock) do
        ...
        if SITE_NAME in plot_sites_list
            ...
            for j in 1:8
                ...
                plot!(p_layers[j], dates_to_plot, sim_profile[:, j], label="Sim(AET_NoComp)", color=:dodgerblue, lw=1.5, ls=:dash,
            ...
            savefig(p_layers, joinpath(plot_dir, "Daily_SM_AET_NoComp_Inversed_$(SITE_NAME).png"))
        end
    end
end
...
CSV.write(joinpath(output_dir, "All_Sites_Metrics_AET_NoComp_Inversed_4to10Months.csv"), df_site_metrics)

report_path = joinpath(output_dir, "Global_Metrics_Report.txt")
open(report_path, "w") do io
    println(io, "=====================================================")
    println(io, "🏆 实际 ET(无强制代偿) + 反演参数 + 连续物理时间流（4-10月）")
    println(io, "=====================================================")
...
println("\n=====================================================")
println("🏆 运行完成！基于实际 ET（废除代偿机制）的连续物理正演已生成。")
