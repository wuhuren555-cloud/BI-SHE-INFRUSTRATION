...
# 终极输出目录
output_dir      = raw"C:\Users\26332\OneDrive\Desktop\kong mission\mechin learning\code\infiltration\Physics_Forward_Inverted_EvalAprOct/"
...
# 2. 读取数据
# ====================================================================
println("📌 [1/4] 正在加载 数据集 与 4-10月专属反演参数表...")

df_old_params = CSV.read(path_old_params, DataFrame)
df_old_params.site = string.(df_old_params.site)
...
valid_sites = intersect(unique(df_smap.site), all_sites_raw, unique(df_old_params.site))
...
# 3. 核心计算 
# ====================================================================
println("⚙️ [2/4] 启动 反演参数正演 (纯正参数注入 + 5分钟微积分装甲)...")
...
Threads.@threads for idx in 1:total_sites
    ...
    try 
        ...
        # 🛡️ 稳压核心 1：强制微积分步长设为 5 分钟 (300秒)
        soil.dt = 300.0 
        
        site_params = df_old_params[df_old_params.site .== SITE_NAME, :]
        
        # 2. 读取并映射反演参数
        for i in 1:soil.N
            z_node = abs(soil.z[i] * 100.0)
            idx_closest_p = argmin(abs.(site_params.depth .- z_node))
            row_p = site_params[idx_closest_p, :]
            ...
            real_max_theta = isempty(valid_vals) ? Float64(row_p.theta_s) : maximum(valid_vals)
            smart_theta_s = max(Float64(row_p.theta_s), real_max_theta + 0.01)
            
            # 🛡️ 稳压核心 2：物理限速器，防止极个别参数越界
            last_K = clamp(Float64(row_p.Ksat), 0.001, 30.0)
            
            soil.param.Ksat[i]  = last_K
            soil.param.θ_sat[i] = smart_theta_s
            soil.param.b[i]     = Float64(row_p.b)
            ψsat_val            = -abs(Float64(row_p.psi_e)) 
        ...
        for t in 2:nrow(df_merged)
            ...
            # 因为 soil.dt 是 300 秒，3小时会切分成 36 步平滑计算
        ...
        lock(lk) do
            ...
            for j in 1:8
                ...
                plot!(p_layers[j], dates_to_plot, sim_profile[:, sim_idx], label="Sim(Inverted)", color=:red, lw=1.2, ls=:dash,
                      title="$(depth)cm | NSE(4-10M):$nv_s | R²(4-10M):$rv_s", ylabel="θ", legend=false, grid=true)
            end
            plot!(p_layers, plot_title="Site: $SITE_NAME (Inverted) | Mean NSE (Apr-Oct): $(round(m_nse, digits=3)) | R²: $(round(m_r2, digits=3))")
            savefig(p_layers, joinpath(plot_dir, "Inverted_EvalAprOct_Final_$(SITE_NAME).png"))
        end
    catch global_e end
end
...
CSV.write(joinpath(output_dir, "All_Sites_Inverted_EvalAprOct_Final_Metrics.csv"), df_site_metrics)

gn = mean(filter(!isnan, df_site_metrics.Mean_NSE))
gr = mean(filter(!isnan, df_site_metrics.Mean_R2))

println("\n=====================================================")
println("🏆 反演参数终极版：真实参数注入 + 4-10月定向评估 报告完毕")
println("=====================================================")
