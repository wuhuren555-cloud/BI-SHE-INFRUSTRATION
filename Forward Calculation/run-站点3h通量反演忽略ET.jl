...
# 🌟 专属输出目录：反演参数 + 无ET + 自由排水全量出图
output_dir     = raw"C:\Users\26332\OneDrive\Desktop\kong mission\mechin learning\code\infiltration\Physics_Forward_反演无ET-改2/"
...
# ====================================================================
# 2. 数据读取与预处理 (剔除 ET, LAI，改为读取反演参数)
# ====================================================================
println("📌 [1/3] 正在加载基础驱动数据 (已加载反演参数，关闭 ET)...")
...
# 仅验证 观测站、下渗预测、反演参数 三者的交集
valid_sites = intersect(unique(df_preds.site), all_sites_raw, unique(df_params.site))
...
# ====================================================================
# 4. 核心循环：反演参数 + 纯下渗 + 纯重力自由排水
# ====================================================================
println("🚀 [3/3] 启动【反演参数 + 纯下渗 + 自由排水】多线程演化 (共 $total_sites 个站点，将全量出图，请耐心等待...)")
    ...
    lock(io_lock) do
        ...
        if SITE_NAME in valid_sites 
            ...
            for j in 1:8
                ...
                plot!(p_layers[j], dates_to_plot, sim_profile[:, j], label="Sim(Inversed_NoET)", color=:crimson, lw=1.5, ls=:dash,
                      title="$(depth)cm | NSE: $nse_val | R²: $r2_val", ylabel="θ", legend=false, grid=true)
            end
            plot!(p_layers, plot_title="Site: $SITE_NAME | Inversed Params | NO ET | Mean NSE: $(isnan(mean_nse) ? "NaN" : round(mean_nse, digits=3))")
            savefig(p_layers, joinpath(plot_dir, "Daily_SM_Inversed_NoET_FreeDrain_$(SITE_NAME).png"))
        end
    end
end
...
CSV.write(joinpath(output_dir, "All_Sites_Metrics_Inversed_NoET_FreeDrain_4to10Months.csv"), df_site_metrics)

println("\n=====================================================")
println("🏆 反演参数纯重力大漏水测试（全量）运行完成！")
